// Vercel/Lambda-compatible handler for the public Fuse Atelier checkout.
// The dynamic /api/[fn] adapter loads handlers from this directory.
...
exports.handler = async (event) => {
  if (event.httpMethod === 'GET') {
    return json(200, { ok: true, configured: !!process.env.PAYSTACK_SECRET_KEY });
  }
  ...
  try {
    ...
    if (!response.ok || !data.status || !data.data || !data.data.authorization_url) {
      return json(502, { error: data.message || 'Could not start Paystack checkout.' });
    }

    return json(200, { authorization_url: data.data.authorization_url });
  } catch (error) {
    return json(502, { error: 'Could not connect to Paystack. Please try again.' });
  }
};
