...
// The model only needs enough choice to design well, not the whole shelf.
// Listing all 117 ids added ~1.5KB of noise to every spec prompt and made the
// model slower without making the typography better — these are the faces that
// actually carry poster work, grouped so it can pick by intent.
const FONT_SHORTLIST = {
  display: ['anton', 'archivo-black', 'bebas-neue', 'oswald', 'barlow-condensed', 'fjalla-one',
            'alfa-slab-one', 'bungee', 'staatliches', 'teko', 'passion-one', 'titan-one'],
  serif:   ['playfair-display', 'abril-fatface', 'bodoni-moda', 'dm-serif-display', 'prata', 'cinzel'],
  script:  ['pacifico', 'caveat', 'permanent-marker', 'great-vibes'],
  mono:    ['space-mono', 'ibm-plex-mono', 'jetbrains-mono'],
  body:    ['inter', 'work-sans', 'karla', 'nunito'],
};
// Anything outside the shortlist is still accepted if the model names it and
// the library has it — the shortlist guides, FONT_IDS enforces.
const FONTS_BY_CATEGORY = FONT_SHORTLIST;
...
{ "id": "hero", "type": "image", "name": "Product", "prompt": "self-contained prompt for THIS subject only — TWO SENTENCES MAXIMUM, what it is and how it is lit", "x": 8, "y": 30, "w": 84, "h": 45, "cutout": true }
...
RULES
- BE CONCISE. Every string you write is time the user waits. Layer names are two words, image prompts are two sentences, and there is no commentary anywhere.
- At most 3 image layers. Flat panels and type do the work; each extra image layer is another generation the user pays for and waits on.
- Bottom-up order. The base fill is the first layer, the topmost detail is the last.
- Between 6 and 12 layers. Fewer than 6 is a template; more than 12 stops being editable and takes too long to write.
