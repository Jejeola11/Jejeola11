...
(interactive?'<script>const orb=document.querySelector("[data-orb]");if(orb&&!matchMedia("(prefers-reduced-motion: reduce)").matches){addEventListener("pointermove",function(e){var x=(e.clientX/innerWidth-.5)*18;var y=(e.clientY/innerHeight-.5)*-18;orb.style.transform="rotateX("+y+"deg) rotateY("+x+"deg)";},{passive:true});}<\/script>':'')+
'</body></html>';
}
