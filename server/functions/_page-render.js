...
function sectionMedia(section){
  ...
  const image=safeUrl(media.image_url);
  const video=safeUrl(media.video_url);
  if(!image&&!video)return '';
  return '<div class="section-media">'+
    (video?'<video src="'+esc(video)+'" autoplay muted loop playsinline preload="metadata" poster="'+esc(image)+'"></video>':'<img src="'+esc(image)+'" alt="" loading="lazy" decoding="async">')+
    '</div>';
}
function renderSection(section){
  ...
  if(type==='testimonials') return '<section class="section">'+head+media+'<div class="quotes">'+quoteCards(section.items)+'</div></section>';
  if(type==='faq') return '<section class="section">'+head+media+'<div class="faq">'+faqCards(section.items)+'</div></section>';
  if(type==='cta') return '<section class="section">'+media+'<div class="cta-block">'+(eyebrow?'<span>'+esc(eyebrow)+'</span>':'')+'<h2>'+esc(title||'Ready for the next step?')+'</h2>'+(body?'<p>'+esc(body)+'</p>':'')+'<a href="#contact">'+esc(text(section.button,'Get started'))+' →</a></div></section>';
  if(type==='immersive') return '<section class="section immersive">'+head+media+'<div class="depth-stage" aria-hidden="true"><i></i><i></i><i></i></div></section>';
  return '<section class="section">'+head+media+'<div class="cards">'+itemCards(section.items)+'</div></section>';
}
...
'.section{padding:86px 0}.section-head{max-width:760px;margin:0 0 30px}.section-head h2,.cta-block h2{margin:8px 0 10px;font-size:clamp(30px,5vw,52px);line-height:1.02;letter-spacing:-1.8px;font-weight:500}.section-head p,.cta-block p{margin:0;color:var(--muted);font-size:15px;line-height:1.65;font-weight:300;max-width:650px}.section-media{width:100%;aspect-ratio:16/8.4;margin:0 0 24px;border-radius:26px;overflow:hidden;border:1px solid var(--line);background:var(--card);box-shadow:0 24px 60px rgba(0,0,0,.18)}.section-media img,.section-media video{width:100%;height:100%;display:block;object-fit:cover}'+
...
(editor?'.fuse-editable{position:relative;cursor:pointer;border-radius:20px;outline:1px dashed transparent;outline-offset:7px}.fuse-editable:hover{outline-color:color-mix(in srgb,var(--accent) 70%,transparent)}.fuse-editable:after{content:"Edit";position:absolute;right:8px;top:8px;z-index:30;padding:6px 9px;border-radius:999px;background:var(--accent);color:#001012;font-size:9px;font-weight:600;letter-spacing:.04em;opacity:0;transform:translateY(-3px);transition:.18s ease;pointer-events:none}.fuse-editable:hover:after{opacity:1;transform:none}.fuse-selected{outline:2px solid var(--accent)!important;outline-offset:7px!important}.hero.fuse-editable:after{top:74px}@media(max-width:800px){.fuse-editable:after{opacity:.82;transform:none}}':'')+
...
'<main><section class="hero wrap'+(editor?' fuse-editable':'')+'"'+(editor?' data-fuse-target="hero"':'')+'><div><span class="eyebrow">'+esc(text(hero.eyebrow,'PREMIUM EXPERIENCE'))+'</span><h1>'+esc(text(hero.headline,'Build something people remember.'))+'</h1><p>'+esc(text(hero.subheadline,'A clear, premium experience designed around the next action.'))+'</p><div class="hero-actions"><a class="primary" href="#contact">'+esc(text(hero.primary_cta,'Get started'))+' →</a>'+(hero.secondary_cta?'<a class="ghost" href="#content">'+esc(hero.secondary_cta)+'</a>':'')+'</div></div>'+
...
'</div></section>'+
'<div id="content" class="wrap">'+sections.map((s,i)=>'<div class="'+(reveal?'reveal ':'')+(editor?'fuse-editable':'')+'"'+(editor?' data-fuse-target="'+esc(s.id||String(i))+'" data-fuse-index="'+i+'"':'')+' style="--i:'+i+'">'+renderSection(s)+'</div>').join('')+'</div>'+
...
(editor?'<script>(function(){function send(el){document.querySelectorAll(".fuse-selected").forEach(x=>x.classList.remove("fuse-selected"));el.classList.add("fuse-selected");parent.postMessage({type:"fuse-page-select",target:el.dataset.fuseTarget||"",index:el.dataset.fuseIndex||null},"*")}document.addEventListener("click",function(e){const el=e.target.closest(".fuse-editable");if(!el)return;e.preventDefault();e.stopPropagation();send(el)},true);window.addEventListener("message",function(e){const d=e.data||{};if(d.type!=="fuse-page-highlight")return;const wanted=String(d.target||"");const el=[...document.querySelectorAll("[data-fuse-target]")].find(x=>x.dataset.fuseTarget===wanted);if(el){document.querySelectorAll(".fuse-selected").forEach(x=>x.classList.remove(".fuse-selected"));el.classList.add("fuse-selected");el.scrollIntoView({behavior:"smooth",block:"center"})}})})();<\/script>':'')+
