...
exports.handler = async (event) => {
  try {
    ...
    try {
      ...
      const msg = e.message || 'Could not start the edit';
      try { await db.rpc('add_credits', { uid: user.id, amount: cost, why: 'refund' }); } catch (_) {}
      try { await db.from('jobs').insert({ request_id: 'failed-' + Date.now() + '-' + Math.random().toString(36).slice(2, 8), user_id: user.id, kind: 'video', model: MODEL, prompt, aspect: '9:16', credits: cost, status: 'failed', error_message: msg }); } catch (_) {}
